...
async function loadPeople() {
    try {
        ...
        specialistsMap = Object.fromEntries(specialists.map(x => [x.id, x.fullName]));
    }
    catch (e) {
        console.error(e);
        setStatus("People load error: " + e.message, true);
    }
}
...
function buildSpecialistControl(order) {
    ...
    specialists.forEach(s => {
        const option = new Option(s.fullName, s.id);
        if (order.specialistId === s.id) option.selected = true;
