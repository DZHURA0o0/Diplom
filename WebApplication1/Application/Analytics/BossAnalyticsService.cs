...
public class BossAnalyticsService
{
        ...
        string? specialistId)
    {
        ...
        return new BossAnalyticsDto
        {
            ...
            AverageEfficiencyPercent = CalculateLeaderShare(specialistAnalytics),
        ...
        int totalCompletedInPeriod)
    {
        ...
        foreach (var specialist in specialists)
        {
            ...
            var complaintRate = Percent(complaintsCount, assignedCount);
        ...
        var best = qualityCandidates
            .Select(item => new
            {
                ...
                Rating =
                    ...
                    0.2 * (100 - item.ComplaintRatePercent)
            })
    ...
    private static double CalculateLeaderShare(
        List<SpecialistAnalyticsDto> specialistAnalytics)
    {
        if (specialistAnalytics.Count == 0)
        {
            return 0;
        }

        return Math.Round(
            specialistAnalytics.Max(item => item.EfficiencyPercent),
            1
        );
    }
